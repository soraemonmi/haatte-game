(() => {
  ...
  const themes = [
    ...
    "拍手", "ため息", "びっくり", "強がり", "謝る", "応援", "告白", "ねるねるねるね"
  ];
  ...
  function handle(m){
    if(m.type==="joined"){
      ...
      $("roomCodeText").textContent=S.room;screen("lobbyScreen");updateLobby(m.roomState);return;
    }
    if(m.type==="room_state"){
      S.hostId=m.roomState.hostId;
    ...
    if(m.type==="game_started"||m.type==="game_update"){
      S.current=m.state;renderGame(m.state);screen("gameScreen");return;
    }
